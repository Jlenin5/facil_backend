...
func NewPurchaseHandler(PurchaseUC *usecase.PurchaseUseCase) *PurchaseHandler {
	return &PurchaseHandler{PurchaseUC: PurchaseUC}
}
...
func (h *PurchaseHandler) CreatePurchase(w http.ResponseWriter, r *http.Request) {
	...
	for i := range sale.PurchaseDetails {
		sale.PurchaseDetails[i].Id = 0
	}

	// ✅ 9. Crear la venta en la base de datos
	err = h.PurchaseUC.CreatePurchase(&sale, sale.PurchaseDetails)
	if err != nil {
...
func (h *PurchaseHandler) GetPurchaseById(w http.ResponseWriter, r *http.Request) {
	...
	sale, err := h.PurchaseUC.GetPurchaseById(id)
	if err != nil {
...
func (h *PurchaseHandler) UpdatePurchase(w http.ResponseWriter, r *http.Request) {
	...
	err = h.PurchaseUC.UpdatePurchase(&sale, sale.PurchaseDetails)
	if err != nil {
...
func (h *PurchaseHandler) OpenPDF(w http.ResponseWriter, r *http.Request) {
	...
	sale, err := h.PurchaseUC.GetPurchaseByBill(bill)
	if err != nil {
		http.Error(w, "Purchase not found", http.StatusNotFound)
		return
	}
	...
	centerText(4, "Arial", "", "--------------------------------------------------------------------", 10)
	// Productos
	for _, detail := range sale.PurchaseDetails {
...
func (h *PurchaseHandler) ExportExcel(w http.ResponseWriter, r *http.Request) {
	...
	if len(requestBody.Ids) > 0 {
		purchases, err = h.PurchaseUC.GetPurchasesByIds(requestBody.Ids)
	} else {
		purchases, err = h.PurchaseUC.GetAllPurchases()
	}
	if err != nil {
	...
	w.Header().Set("Content-Disposition", "attachment; filename=purchases-export.xlsx")
	err = file.Write(w)
	if err != nil {
