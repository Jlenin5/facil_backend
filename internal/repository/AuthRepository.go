...
func querySelectUser(whereClause string) string {
	query := fmt.Sprintf(`
		SELECT 
			...
			COALESCE(e.id, 0) AS "employee.id", COALESCE(e.first_name, '') AS "employee.first_name", COALESCE(e.second_name, '') AS "employee.second_name", COALESCE(e.third_name, '') AS "employee.third_name", COALESCE(e.surname, '') AS "employee.surname", COALESCE(e.second_surname, '') AS "employee.second_surname", COALESCE(e.document_number, '') AS "employee.document_number", COALESCE(e.warehouse_id, 0) AS "employee.warehouse_id", COALESCE(e.job_position_id, 0) AS "employee.job_position_id", COALESCE(e.status, B'0') AS "employee.status",
